...
public class ActiveHandlers {
     ...
     */
    public synchronized void sendMessageToAll(SocketHandler sender, String message) {
        for (SocketHandler handler : activeHandlersSet) // pro vechny aktivní handlery
            if (handler != sender) {
                if (!handler.offerMessage(message)) // zkus pøidat zprávu do fronty jeho zpráv
    ...
    public synchronized Set<SocketHandler> get() {
        return new HashSet<>(activeHandlersSet);
    }
}
